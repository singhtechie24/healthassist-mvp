          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="w-full max-w-md">
              {!user ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <button 
                      onClick={async () => {
                        if (user) {
                          try {
                            await VoiceChatLimitsService.forceResetUserLimits(user.uid);
                            await loadVoiceChatLimits(user.uid);
                          } catch (error) {
                            console.error('Error resetting voice chat limits:', error);
                          }
                        }
                      }}
                ...
                // Voice chat available (with limits)
                <div className="w-full">
                  <div className="text-center mb-4 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                    ...
                    <button 
                      ...
                      title="Reset your voice chat limits"
                    >
                      🔄 Reset My Limits
                    </button>
                  </div>
                  <RealtimeVoiceChat 
                    ...
                    remainingConversations={voiceChatUsage?.remainingToday || 0}
                    maxConversations={voiceChatUsage?.dailyLimit || 5}
                  />
                </div>
              )}
            </div>
