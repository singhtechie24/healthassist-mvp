...
export default function Mood() {
  ...
  const [displayMood, setDisplayMood] = useState<number>(5); // For GSAP animation
  ...
  const moodValueRef = useRef<HTMLDivElement>(null);
  const moodDescRef = useRef<HTMLDivElement>(null);
  ...
  // GSAP smooth mood animation
  const animateToMood = (targetMood: number) => {
    // Animate the display mood value smoothly
    gsap.to({ value: displayMood }, {
      value: targetMood,
      duration: 0.6,
      ease: "power2.out",
      onUpdate: function() {
        setDisplayMood(Math.round(this.targets()[0].value));
      }
    });

    // Animate emoji with bounce and scale
    if (emojiRef.current) {
    ...
    // Animate mood value with elastic effect
    if (moodValueRef.current) {
      gsap.fromTo(moodValueRef.current,
        { scale: 1, y: 0 },
        {
          scale: 1.1,
          y: -5,
    ...
    // Animate description with fade transition
    if (moodDescRef.current) {
      gsap.fromTo(moodDescRef.current,
        { opacity: 1, y: 0 },
        {
          opacity: 0,
          y: -10,
  ...
  // Handle slider change with GSAP animation
  const handleSliderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newValue = parseInt(e.target.value);
    setCurrentMood(newValue);
    animateToMood(newValue);
  };

  // Handle keyboard navigation with smooth animation
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') {
      e.preventDefault();
      const newValue = Math.max(1, currentMood - 1);
      setCurrentMood(newValue);
      animateToMood(newValue);
    } else if (e.key === 'ArrowRight' || e.key === 'ArrowUp') {
      e.preventDefault();
      const newValue = Math.min(10, currentMood + 1);
      setCurrentMood(newValue);
      animateToMood(newValue);
    }
  };
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl border border-white/20 shadow-2xl p-8 mb-8 transition-all duration-300 hover:shadow-3xl">
          <div className="text-center mb-8">
           ...
           <div className="text-center mb-8">
             ...
             >
               {moodDescriptions[displayMood - 1]}
             </div>
           </div>
           ...
           <div className="mb-8">
             <div className="relative">
               <input
                 ...
                 className="w-full h-3 rounded-full appearance-none cursor-pointer transition-all duration-300 hover:shadow-lg mood-slider focus:outline-none focus:ring-4 focus:ring-blue-300/30"
                 style={{
